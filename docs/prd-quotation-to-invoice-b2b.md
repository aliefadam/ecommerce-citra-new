...
- **[Baru] Jalur langsung (tagih/bayar dulu, kirim belakangan)**: admin memilih satu Sales Order dan memilih item + qty langsung dari `sales_order_detail_id` (mirip pola pemilihan item Proforma Invoice), tanpa perlu ada Surat Jalan sama sekali saat Invoice diterbitkan. Tidak ada validasi qty invoice terhadap qty yang nantinya benar-benar dikirim pada fase pertama (rekonsiliasi jumlah ditunda ke fase berikutnya) — qty yang divalidasi adalah **sisa qty yang belum ter-invoice** (`SalesOrderDetail::remainingToInvoiceDirect()`, dihitung dari `quantity` dikurangi total qty yang sudah masuk ke Invoice aktif manapun — baik lewat jalur langsung maupun lewat Surat Jalan), bukan `quantity` penuh, supaya qty yang sama tidak bisa ditagih dobel lewat invoice langsung berkali-kali. Tombol "Buat Invoice Langsung" di halaman Sales Order otomatis hilang begitu seluruh item sudah ter-invoice penuh (`SalesOrder::hasRemainingQtyToInvoiceDirect()` bernilai false).
...
- **[Bug ditemukan & diperbaiki, dilaporkan user]** Tombol "Buat Invoice Langsung" di halaman Sales Order tetap tampil dan bisa diklik meskipun Sales Order tersebut **sudah fully invoiced** (semua item sudah tercakup Invoice aktif lain, baik lewat Surat Jalan maupun lewat Invoice langsung sebelumnya) — implementasi awal hanya mengecek `$salesOrder->status !== 'cancelled'`, tanpa mengecek sisa qty yang belum ter-invoice sama sekali. Konsekuensinya: admin bisa membuat Invoice langsung baru untuk qty yang sama berkali-kali (dobel tagih), karena validasi qty di `storeDirect()` saat itu hanya membandingkan terhadap `quantity` penuh pada Sales Order detail, bukan terhadap sisa yang belum ter-invoice. Diperbaiki dengan menambahkan `SalesOrderDetail::quantityInvoiced()`/`remainingToInvoiceDirect()` (menghitung total qty yang sudah masuk Invoice aktif manapun, lewat jalur langsung maupun Surat Jalan) dan `SalesOrder::hasRemainingQtyToInvoiceDirect()` — dipakai untuk menyembunyikan tombol, menolak akses form (`createDirectForm()` redirect jika tidak ada sisa), dan memvalidasi qty di `storeDirect()` terhadap sisa yang benar (bukan qty penuh).
