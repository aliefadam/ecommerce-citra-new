...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: Arial, Helvetica, sans-serif; }
        body { margin: 0; background: #f1f5f9; color: #000; }
        ...
        .label { width: 100mm; min-height: 148mm; margin: 0 auto 24px; background: #fff; border: 2px solid #222; }
        .row { display: flex; }
        .cell { border-top: 1.5px solid #555; padding: 7px 9px; }
        .top { height: 19mm; display: flex; align-items: center; justify-content: space-between; padding: 7px 10px; }
        .brand { font-size: 22px; font-weight: 900; letter-spacing: -.04em; color: #27356f; }
        .brand small { display: block; color: #555; font-size: 9px; letter-spacing: 0; font-weight: 700; margin-top: -2px; }
        .courier { font-size: 24px; font-weight: 900; font-style: italic; color: #203a82; text-align: right; line-height: .8; }
        .courier span { display: block; color: #e11d48; font-size: 9px; font-style: normal; letter-spacing: .08em; margin-top: 5px; }
        .barcode-wrap { border-top: 1.5px solid #555; padding: 8px 8px 6px; text-align: center; }
        .barcode { height: 17mm; display: flex; align-items: stretch; justify-content: center; gap: 1px; overflow: hidden; }
        .bar { display: block; background: #000; height: 100%; }
        .resi { font-size: 15px; font-weight: 900; margin-top: 5px; }
        .total { border-top: 1.5px solid #555; padding: 8px 8px; text-align: center; font-size: 24px; font-weight: 900; }
        .service { border-top: 1.5px solid #555; padding: 5px 8px; text-align: center; font-size: 15px; font-weight: 900; }
        .half { width: 50%; font-size: 12px; font-weight: 800; min-height: 12mm; }
        .half + .half { border-left: 1.5px solid #222; }
        .items { font-size: 12px; font-weight: 900; line-height: 1.25; }
        .party { width: 50%; padding: 10px 9px; min-height: 43mm; }
        .party + .party { border-left: 0; }
        .title { font-size: 13px; font-weight: 900; margin-bottom: 3px; }
        .name { font-size: 12px; font-weight: 900; line-height: 1.05; margin-bottom: 9px; }
        .address { font-size: 12px; font-weight: 800; line-height: 1.08; white-space: pre-line; }
        .phone { font-size: 12px; font-weight: 900; line-height: 1.08; margin-top: 10px; }
        .notes { min-height: 17mm; font-size: 11px; line-height: 1.2; }
        .foot { font-size: 10px; font-style: italic; line-height: 1.25; }
        @page { size: 100mm 148mm; margin: 0; }
        @media print {
            body { background: #fff; }
            .actions { display: none; }
            .label { width: 100mm; min-height: 148mm; margin: 0; page-break-after: always; }
        }
    </style>
</head>
<body>
@php
    ...
    $senderAddress = $storeLocation
        ? trim(($storeLocation->label ?: 'Lokasi Toko') . "\n" . ($storeLocation->city_name ?: '') . ($storeLocation->province_name ? ', ' . $storeLocation->province_name : ''))
