    ...
    <body class="@yield('body_class', 'bg-stone-50 text-slate-800 pb-20 md:pb-0')">
        ...
        <nav class="ec-bottom-nav fixed inset-x-0 bottom-0 border-t border-slate-200 bg-white/95 px-2 shadow-[0_-8px_30px_rgb(17_24_32/.08)] backdrop-blur md:hidden" aria-label="Navigasi cepat">
            <div class="mx-auto grid max-w-md grid-cols-5">
                ...
                <a href="{{ route('frontend.cart') }}"
                    class="ec-bottom-link relative" @if($navIsCart) aria-current="page" @endif>
        ...
        @if (!empty($appStoreSettings['social_whatsapp']))
            <a href="{{ $appStoreSettings['social_whatsapp'] }}" target="_blank" rel="noopener noreferrer"
                class="ec-chat hidden md:flex fixed bottom-6 left-6 items-center justify-center w-14 h-14 bg-green-700 hover:bg-green-800 active:scale-95 text-white rounded-xl shadow-lg transition-all duration-200"
