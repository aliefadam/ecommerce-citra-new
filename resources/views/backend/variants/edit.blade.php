...
@section('content')
    <main class="flex-1 p-4 sm:p-6 mt-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Variant</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Perbarui data varian yang dipilih.</p>
        </div>
        ...
        <div class="max-w-3xl bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <form action="{{ route('variants.update', $variant) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $variant->name) }}" placeholder="Contoh: Warna, Ukuran, Kapasitas"
                ...
                <div>
                    ...
                    <input type="text" name="value" value="{{ old('value', $variant->value) }}" placeholder="Contoh: Merah, XL, 128GB"
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit"
                        class="px-4 py-2.5 text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors">Update Variant</button>
                </div>
            </form>
        </div>
