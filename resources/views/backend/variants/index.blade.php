...
@section('content')
    <main class="flex-1 p-4 sm:p-6 mt-6">
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Variant Management</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola master varian produk (tipe & nilai).</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="flex flex-col sm:flex-row gap-3 p-4 border-b border-slate-200 dark:border-slate-700">
                <div class="relative flex-1">
                    ...
                    <input id="variantTableSearch" type="text" placeholder="Search tipe or nilai..."
                    ...
                    <thead class="bg-slate-50 dark:bg-slate-700/50">
                        <tr>
                            ...
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 cursor-pointer select-none"
                                onclick="sortVariantTable(1)">Tipe <span class="text-slate-300 dark:text-slate-600">↕</span>
                            </th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 cursor-pointer select-none"
                                onclick="sortVariantTable(2)">Nilai <span class="text-slate-300 dark:text-slate-600">↕</span>
                            </th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="variantTableBody" class="divide-y divide-slate-100 dark:divide-slate-700/60"></tbody>
                </table>
            </div>
            ...
            <div
                ...
                <p id="variantPaginationInfo" class="text-sm text-slate-500 dark:text-slate-400"></p>
                <div class="flex items-center gap-1" id="variantPaginationButtons"></div>
            </div>
        </div>
        ...
        <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
            ...
            <div
                ...
                <h3 class="font-bold text-lg text-slate-800 dark:text-white mb-2">Delete Variant?</h3>
...
@section('script')
    @php
        $variantItems = $variants->map(fn($v) => ['id' => $v->id, 'name' => $v->name, 'value' => $v->value])->values()->all();
    @endphp
    <script>
        const variantItems = @json($variantItems);
        const editUrlTemplate = @json(route('variants.edit', ['variant' => '__ID__']));
        const deleteUrlTemplate = @json(route('variants.destroy', ['variant' => '__ID__']));
        const csrfToken = @json(csrf_token());

        let variantTableApi = null;
        let deletingFormId = null;

        function buildVariantRow(variant, visibleIndex) {
            const editUrl = editUrlTemplate.replace('__ID__', variant.id);
            const deleteUrl = deleteUrlTemplate.replace('__ID__', variant.id);
            const formId = `delete-variant-${variant.id}`;
            return `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                    <td class="px-4 py-3.5 text-slate-500 dark:text-slate-400">${visibleIndex + 1}</td>
                    <td class="px-4 py-3.5 font-medium text-slate-800 dark:text-slate-200">${variant.name}</td>
                    <td class="px-4 py-3.5 text-slate-600 dark:text-slate-300">${variant.value}</td>
        ...
        function sortVariantTable(col) { variantTableApi?.sortBy(col); }
        ...
        variantTableApi = initAdminDataTable({
            data: variantItems,
            perPage: 10,
            itemLabel: 'variants',
            searchInputId: 'variantTableSearch',
            tbodyId: 'variantTableBody',
            paginationInfoId: 'variantPaginationInfo',
            paginationButtonsId: 'variantPaginationButtons',
            ...
            renderRow: (variant, index) => buildVariantRow(variant, index),
            emptyRowHtml: '<tr><td colspan="4" class="text-center py-12 text-slate-400 dark:text-slate-500">No variants found</td></tr>',
        });
