...
@section('content')
    <main class="flex-1 p-4 sm:p-6 mt-6">
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Main Categories</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola kategori utama produk.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="flex flex-col sm:flex-row gap-3 p-4 border-b border-slate-200 dark:border-slate-700">
                <div class="relative flex-1">
                    ...
                    <input id="mainCatSearch" type="text" placeholder="Search main category..."
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm" data-mobile-cards data-mobile-primary-column="3">
                    ...
                    <tbody id="mainCatTableBody" class="divide-y divide-slate-100 dark:divide-slate-700/60"></tbody>
                </table>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-slate-200 dark:border-slate-700">
                <p id="mainCatPaginationInfo" class="text-sm text-slate-500 dark:text-slate-400"></p>
                <div class="flex items-center gap-1" id="mainCatPaginationButtons"></div>
            </div>
        </div>
        ...
        <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm border border-slate-200 dark:border-slate-700 p-6 text-center">
                ...
                <h3 class="font-bold text-lg text-slate-800 dark:text-white mb-2">Delete Main Category?</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">This action cannot be undone. Are you sure you want to delete this category?</p>
...
@section('script')
    ...
    @endphp
    <script>
        const mainCatItems = @json($mainCatItems);
        const editUrlTemplate   = @json(route('main-categories.edit',   ['main_category' => '__ID__']));
        const deleteUrlTemplate = @json(route('main-categories.destroy', ['main_category' => '__ID__']));
        const csrfToken = @json(csrf_token());

        let mainCatTableApi = null;
        let deletingFormId  = null;

        function buildMainCatRow(item, visibleIndex) {
            const editUrl   = editUrlTemplate.replace('__ID__', item.id);
            const deleteUrl = deleteUrlTemplate.replace('__ID__', item.id);
            const formId    = `delete-maincat-${item.id}`;
        ...
        function sortMainCatTable(col) { mainCatTableApi?.sortBy(col); }
        ...
        mainCatTableApi = initAdminDataTable({
            data: mainCatItems,
            perPage: 10,
            itemLabel: 'main categories',
            searchInputId: 'mainCatSearch',
            tbodyId: 'mainCatTableBody',
            paginationInfoId: 'mainCatPaginationInfo',
            paginationButtonsId: 'mainCatPaginationButtons',
            searchFields: ['name'],
            filters: [],
            sortMap: { 1: (c) => c.name },
