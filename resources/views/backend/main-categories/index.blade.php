    ...
    <main class="flex-1 p-4 sm:p-6 mt-6">
        <div class="mb-6 flex items-center justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola kategori utama.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('main-categories.edit', $item) }}"
                                        class="text-blue-600 hover:text-blue-700 text-xs font-semibold">Edit</a>
                                    <form action="{{ route('main-categories.destroy', $item) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                            onclick="return confirm('Hapus kategori utama ini?')"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-10 text-slate-400">Belum ada data</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
