...
@section('content')
    @php
        $oldCategoryType = old('category_detail_id') ? 'detail' : (old('main_category_id') ? 'main' : 'category');
        $oldCatId = old('category_detail_id', old('main_category_id', old('category_id')));
        ...
        $oldVariants = collect(old('variants', []))
            ->map(function ($v) use ($attributeDefinitions) {
