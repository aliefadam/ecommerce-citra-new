        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="flex flex-col sm:flex-row gap-3 p-4 border-b border-slate-200 dark:border-slate-700">
                <div class="relative flex-1">
                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 dark:bg-slate-700/50">
                        <tr>
                            ...
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 cursor-pointer select-none" onclick="sortProductTable(1)">Name <span class="text-slate-300 dark:text-slate-600">↕</span></th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 cursor-pointer select-none" onclick="sortProductTable(2)">Category <span class="text-slate-300 dark:text-slate-600">↕</span></th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400">Variants</th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 cursor-pointer select-none" onclick="sortProductTable(4)">Status <span class="text-slate-300 dark:text-slate-600">↕</span></th>
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 dark:border-slate-700">
                ...
                <div class="flex items-start justify-between p-6 pb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Upload file sesuai template untuk import produk & varian</p>
                        </div>
                    </div>
                    <button type="button" onclick="closeImportModal()" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors ml-2 mt-0.5">
                ...
                <div class="px-6 pb-6 space-y-4">
                    ...
                    <a href="{{ route('products.import-template') }}"
                        ...
                        <div class="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-800/50 flex items-center justify-center shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-blue-700 dark:text-blue-300 group-hover:text-blue-800 dark:group-hover:text-blue-200">Download Template Excel</p>
                            <p class="text-xs text-blue-500 dark:text-blue-400">Belum punya template? Unduh di sini</p>
                        </div>
                        <svg class="w-4 h-4 text-blue-400 dark:text-blue-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                    </a>
                    ...
                    <form action="{{ route('products.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        ...
                        {{-- Custom file input --}}
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">File Excel (.xlsx / .xls)</label>
                            <input id="import_file" name="import_file" type="file" accept=".xlsx,.xls" class="hidden" onchange="handleFileChange(this)" />
                            <label for="import_file" id="file-drop-area"
                                ...
                                <div id="file-icon-wrap" class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
                                    <svg class="w-5 h-5 text-slate-400 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                    </svg>
                                </div>
                                <div id="file-placeholder" class="text-center">
                                    <p class="text-sm font-semibold text-slate-600 dark:text-slate-300">Klik untuk pilih file</p>
                                    <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">atau drag & drop di sini</p>
                                </div>
                                <div id="file-chosen" class="hidden text-center">
                                    <p id="file-name" class="text-sm font-semibold text-emerald-700 dark:text-emerald-400 break-all"></p>
...
@section('script')
    @php
        $productItems = $products->map(function ($p) {
            return [
                'id'             => $p->id,
                'name'           => $p->name,
    ...
    <script>
        ...
        function sortProductTable(col) { productTableApi?.sortBy(col); }
        ...
        function handleFileChange(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                iconWrap.innerHTML = `<svg class="w-5 h-5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`;
                iconWrap.className = 'w-10 h-10 rounded-full bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center';
        ...
        function resetFileInput() {
            ...
            iconWrap.innerHTML = `<svg class="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>`;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const dropArea = document.getElementById('file-drop-area');
            if (!dropArea) return;
            ['dragenter', 'dragover'].forEach(evt => dropArea.addEventListener(evt, e => { e.preventDefault(); dropArea.classList.add('border-emerald-400'); }));
            ['dragleave', 'drop'].forEach(evt => dropArea.addEventListener(evt, e => { e.preventDefault(); dropArea.classList.remove('border-emerald-400'); }));
        ...
        productTableApi = initAdminDataTable({
            ...
            filters: [{ elementId: 'productStatusFilter', field: 'status' }],
            sortMap: {
                1: (p) => p.name,
                2: (p) => p.category,
                4: (p) => p.status,
