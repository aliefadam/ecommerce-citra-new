    ...
    <main class="flex-1 p-4 sm:p-6 mt-6">
        ...
        <div class="space-y-6">
            @foreach ($groups as $group)
                ...
                <section class="rounded-3xl border border-sky-100 bg-white p-5 sm:p-7 shadow-sm dark:bg-slate-800 dark:border-slate-700">
                    <div class="flex items-start gap-4">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl {{ $groupTone['box'] }}">
                            <i data-lucide="{{ $group['icon'] }}" class="h-6 w-6"></i>
                        </div>
                        <div>
                            <h2 class="text-xl font-extrabold text-slate-800 dark:text-white">{{ $group['title'] }}</h2>
                    ...
                    <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        @foreach ($group['items'] as $item)
                            ...
                            <a href="{{ $item['route'] }}"
                                class="group flex min-h-[128px] items-center gap-4 rounded-2xl border border-sky-100 bg-white p-4 transition-all hover:-translate-y-0.5 hover:shadow-lg {{ $tone['border'] }} dark:bg-slate-900/30 dark:border-slate-700">
                                <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl {{ $tone['box'] }}">
                                ...
                                <i data-lucide="arrow-right" class="h-5 w-5 shrink-0 text-slate-400 transition-transform group-hover:translate-x-1 group-hover:text-blue-600"></i>
                            </a>
                        @endforeach
                    </div>
                </section>
