...
@section('content')
    <main class="flex-1 p-4 sm:p-6 mt-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Create Banner</h1>
                ...
                <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-4">
                    <div>
                        ...
                        <select name="type" class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-slate-200">
                            <option value="carousel" @selected(old('type', 'carousel') === 'carousel')>Carousel — Slider utama (kiri)</option>
                            <option value="side" @selected(old('type') === 'side')>Side — Banner kanan (maks 2)</option>
                        </select>
                        @error('type')
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Upload Gambar</label>
                        <div class="flex items-center gap-3">
                            <div id="bannerImagePreviewWrap" class="hidden shrink-0">
                                <img id="bannerImagePreview" src="" alt="Preview Banner"
                        ...
                        <p id="bannerSizeHint" class="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
                            Rekomendasi: <span id="bannerSizeHintText">1600 × 700 px (rasio 16:4, untuk slider utama)</span> — otomatis dikonversi saat upload.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="target_url" value="{{ old('target_url') }}" placeholder="https://example.com"
                            class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-slate-200" />
                        @error('target_url')
                            <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-5">
                    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-4">
                        ...
                        <label class="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                            ...
                            <input type="checkbox" name="is_active" value="1" class="accent-blue-500" @checked(old('is_active', 1)) />
                            Active
                        </label>
                        @error('is_active')
