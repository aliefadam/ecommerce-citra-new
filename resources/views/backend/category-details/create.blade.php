        ...
        <div class="max-w-3xl bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <form action="{{ route('category-details.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="specification_template_id" class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">Tanpa template (kompatibilitas lama)</option>
                        @foreach ($specificationTemplates as $template)
                            <option value="{{ $template->id }}" @selected((string) old('specification_template_id') === (string) $template->id)>{{ $template->name }}</option>
                        @endforeach
                    </select>
                    <p class="mt-1 text-xs text-slate-500">Menentukan field teknis yang tampil pada form produk dan storefront.</p>
                </div>
