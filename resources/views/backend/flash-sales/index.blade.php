...
@section('content')
    <main class="flex-1 p-4 sm:p-6 mt-6">
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Flash Sale Management</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola campaign flash sale dan item promo.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="flex flex-col sm:flex-row gap-3 p-4 border-b border-slate-200 dark:border-slate-700">
                <div class="relative flex-1">
                    ...
                    <input id="flashSaleTableSearch" type="text" placeholder="Search nama flash sale..."
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 dark:bg-slate-700/50">
                        <tr>
                            ...
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400">Periode</th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400">Status</th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400">Items</th>
                            <th class="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="flashSaleTableBody" class="divide-y divide-slate-100 dark:divide-slate-700/60"></tbody>
                </table>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-slate-200 dark:border-slate-700">
                <p id="flashSalePaginationInfo" class="text-sm text-slate-500 dark:text-slate-400"></p>
                <div class="flex items-center gap-1" id="flashSalePaginationButtons"></div>
            </div>
        </div>
        ...
        <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm border border-slate-200 dark:border-slate-700 p-6 text-center">
                ...
                <h3 class="font-bold text-lg text-slate-800 dark:text-white mb-2">Delete Flash Sale?</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Tindakan ini tidak dapat dibatalkan.</p>
...
@section('script')
        ...
        $flashSaleItems = $flashSales
            ->map(function ($sale) {
                return [
                    'id' => $sale->id,
                    'name' => $sale->name,
                    'start_at' => $sale->start_at?->format('d M Y H:i'),
                    'end_at' => $sale->end_at?->format('d M Y H:i'),
                    'status' => $sale->status,
    ...
    @endphp
    <script>
        const flashSaleItems = @json($flashSaleItems);
        const editUrlTemplate = @json(route('flash-sales.edit', ['flash_sale' => '__ID__']));
        const deleteUrlTemplate = @json(route('flash-sales.destroy', ['flash_sale' => '__ID__']));
        const csrfToken = @json(csrf_token());
        let flashSaleTableApi = null;
        let deletingFormId = null;

        function statusBadge(status) {
            if (status === 'active') return '<span class="px-2 py-1 rounded-lg text-xs font-semibold bg-emerald-100 text-emerald-700">Active</span>';
            if (status === 'inactive') return '<span class="px-2 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-600">Inactive</span>';
            return '<span class="px-2 py-1 rounded-lg text-xs font-semibold bg-amber-100 text-amber-700">Draft</span>';
        }

        function buildFlashSaleRow(item, visibleIndex) {
            const editUrl = editUrlTemplate.replace('__ID__', item.id);
            const deleteUrl = deleteUrlTemplate.replace('__ID__', item.id);
            const formId = `delete-flash-sale-${item.id}`;
            return `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                    ...
                    <td class="px-4 py-3.5 text-slate-600 dark:text-slate-300">${item.start_at} - ${item.end_at}</td>
                    <td class="px-4 py-3.5">${statusBadge(item.status)}</td>
                    <td class="px-4 py-3.5 text-slate-600 dark:text-slate-300">${item.items_count} item</td>
        ...
        flashSaleTableApi = initAdminDataTable({
            data: flashSaleItems,
            perPage: 10,
            itemLabel: 'flash sales',
            searchInputId: 'flashSaleTableSearch',
            tbodyId: 'flashSaleTableBody',
            paginationInfoId: 'flashSalePaginationInfo',
            paginationButtonsId: 'flashSalePaginationButtons',
            searchFields: ['name', 'status', 'start_at', 'end_at'],
            filters: [],
            sortMap: {},
            renderRow: (item, index) => buildFlashSaleRow(item, index),
            emptyRowHtml: '<tr><td colspan="6" class="text-center py-12 text-slate-400 dark:text-slate-500">No flash sales found</td></tr>',
        });
