...
@section('content')
    ...
    <main class="flex-1 p-4 sm:p-6 mt-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Flash Sale</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Perbarui campaign flash sale beserta item promonya.
            </p>
        </div>

        <form action="{{ route('flash-sales.update', $flashSale) }}" method="POST" x-data="flashSaleForm({ oldItems: {{ json_encode($oldItems) }} })">
            @csrf
            @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nama
                                    Campaign</label>
                                <input type="text" name="name" value="{{ old('name', $flashSale->name) }}"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="notes" rows="3" placeholder="Catatan opsional..."
                                    class="w-full px-4 py-2.5 text-sm rounded-xl focus:outline-none focus:ring-2 dark:text-slate-200 placeholder-slate-400 {{ $errors->has('notes') ? 'border-2 border-red-400 bg-red-50 dark:bg-red-900/10 dark:border-red-600 focus:ring-red-400' : 'border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 focus:ring-blue-500' }}">{{ old('notes', $flashSale->notes) }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-5">
                    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
                        ...
                        <select name="status"
                            ...
                            <option value="active" @selected(old('status', $flashSale->status) === 'active')>Active</option>
                            <option value="inactive" @selected(old('status', $flashSale->status) === 'inactive')>Inactive</option>
                            <option value="draft" @selected(old('status', $flashSale->status) === 'draft')>Draft</option>
                        </select>
                    </div>
