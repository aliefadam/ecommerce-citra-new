...
@section('content')
    <main class="flex-1 p-4 sm:p-6 mt-6">
        ...
        <form method="POST" action="{{ route('quotations.convert', $quotation) }}" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
            @csrf
                ...
                <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800 overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-100 dark:divide-slate-700/60 text-sm">
                        <div class="flex items-center justify-between gap-3 px-5 py-3">
                            ...
                            <span id="convertSummarySubtotal" class="font-semibold text-slate-700 dark:text-slate-200">Rp 0</span>
                        </div>

                        <div class="flex items-center gap-3 px-5 py-3">
                            <label for="convert_ppn_rate" class="shrink-0 text-slate-500 dark:text-slate-400 w-20">PPN (%)</label>
                            <input id="convert_ppn_rate" name="ppn_rate" type="number" min="0" max="100" step="0.01"
                                value="{{ old('ppn_rate', $defaultPpnRate) }}" oninput="recalculateConvert()"
                            ...
                            <span id="convertSummaryPpn" class="shrink-0 flex-1 text-right font-semibold text-slate-700 dark:text-slate-200">Rp 0</span>
                        </div>

                        <div class="flex items-center gap-3 px-5 py-3">
                            <label for="convert_shipping_cost" class="shrink-0 text-slate-500 dark:text-slate-400 w-20">Ongkir</label>
                            <div class="relative flex-1">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400">Rp</span>
                                <input id="convert_shipping_cost" name="shipping_cost" type="number" min="0" step="1"
                                    value="{{ old('shipping_cost', 0) }}" oninput="recalculateConvert()"
                        ...
                        <div class="flex items-center gap-3 px-5 py-3">
                            <label for="convert_admin_fee" class="shrink-0 text-slate-500 dark:text-slate-400 w-20">Biaya Admin</label>
                            <div class="relative flex-1">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400">Rp</span>
                                <input id="convert_admin_fee" name="admin_fee" type="number" min="0" step="1"
                                    value="{{ old('admin_fee', 0) }}" oninput="recalculateConvert()"
                                    class="w-full rounded-lg border border-slate-200 bg-slate-50 pl-8 pr-3 py-1.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200">
                            </div>
                        </div>

                        <div class="flex items-center gap-3 px-5 py-3">
                            <label for="convert_other_cost" class="shrink-0 text-slate-500 dark:text-slate-400 w-20">Lain-lain</label>
                            <div class="relative flex-1">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400">Rp</span>
                                <input id="convert_other_cost" name="other_cost" type="number" min="0" step="1"
                                    value="{{ old('other_cost', 0) }}" oninput="recalculateConvert()"
                                    class="w-full rounded-lg border border-slate-200 bg-slate-50 pl-8 pr-3 py-1.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200">
                            </div>
                        </div>
...
@section('script')
    <script>
        function recalculateConvert() {
            ...
            document.getElementById('convertSummarySubtotal').textContent = 'Rp ' + subtotal.toLocaleString('id-ID');
            document.getElementById('convertSummaryPpn').textContent      = 'Rp ' + ppnAmount.toLocaleString('id-ID');
